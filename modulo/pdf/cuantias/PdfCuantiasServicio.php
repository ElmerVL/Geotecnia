...
class PdfCuantiasServicio
{
    function imprimir_formulario($idSolicitud, $tipo_proyecto, $idCliente) {
        if ($tipo_proyecto === 1) {
            $formularioELModelo = new FormularioELModelo();
            $formularioELModelo->setClienteIdCliente($idCliente);
            $formularioELModelo->setEnsayoLaboratorioSolicitudIdSolicitud($idSolicitud);

            $formularioELDao = new FormularioELDAO();
            $detalleFormulario = $formularioELDao->getDetalleFormularioELDAO($formularioELModelo);
        } else {
            $formularioTCModelo = new FormularioTCModelo();
            $formularioTCModelo->setClienteIdCliente($idCliente);
            $formularioTCModelo->setTrabajoCampoSolicitudIdSolicitud($idSolicitud);

            $formularioTCDao = new FormularioTCDAO();
            $detalleFormulario = $formularioTCDao->getDetalleFormularioTCDAO($formularioTCModelo);
        }

        $tipo_institucion = $detalleFormulario[10];
         $precio = $detalleFormulario[9];

        $modelo_cuantias = new PdfCuantiasServicio();

        if ($tipo_proyecto == 1) {
            if ($precio <= 10000) {
                if ($tipo_institucion == 'privada') {
                    $modelo_cuantias->imprimir_compromiso_pago($idSolicitud, $detalleFormulario);
                } else {
                    $modelo_cuantias->imprimir_nota_aceptacion($idSolicitud, 20, $tipo_proyecto, $detalleFormulario);
                }
            } else {
                if ($tipo_institucion == 'privada') {
                    $modelo_cuantias->imprimir_nota_aceptacion($idSolicitud, 50, $tipo_proyecto, $detalleFormulario);
                } else {
                    $modelo_cuantias->imprimir_nota_aceptacion($idSolicitud, 20, $tipo_proyecto, $detalleFormulario);
                }
            }
            ...
            if ($tipo_institucion == 'privada') {
                $modelo_cuantias->imprimir_nota_aceptacion_tc($idSolicitud, 50, $tipo_proyecto, $detalleFormulario);
            } else {
                $modelo_cuantias->imprimir_nota_aceptacion_tc($idSolicitud, 20, $tipo_proyecto, $detalleFormulario);
            }
        }
    }

    function imprimir_compromiso_pago($idSolicitud, $detalleFormulario) {
        $detalleEnsayoDao = new DetalleEnsayoDAO();
        $array_datos_ensayos = $detalleEnsayoDao->getDetalleEnsayos($idSolicitud);
        ...
        $pdf->SetXY(80, 35);
        $pdf->MultiCell(120, 7,utf8_decode("$detalleFormulario[0]") , 1, 'L', FALSE);
        $pdf->SetXY(10, 42);
        $pdf->MultiCell(70, 6,utf8_decode('Facturación a nombre de (razón social de la empresa o nombre del particular):') , 1, 'L', FALSE);
        $pdf->SetXY(80, 42);
        $pdf->MultiCell(120, 12,utf8_decode("$detalleFormulario[1]") , 1, 'L', FALSE);
        $pdf->SetXY(10, 54);
        $pdf->MultiCell(70, 7,utf8_decode('Nro. de NIT o Nro. de CI:') , 1, 'L', FALSE);
        $pdf->SetXY(80, 54);
        $pdf->MultiCell(120, 7,utf8_decode("$detalleFormulario[2]") , 1, 'L', FALSE);
        $pdf->SetXY(10, 61);
        $pdf->MultiCell(70, 7,utf8_decode('Nombre de la persona de contacto:') , 1, 'L', FALSE);
        $pdf->SetXY(80, 61);
        $pdf->MultiCell(120, 7,utf8_decode("$detalleFormulario[3]") , 1, 'L', FALSE);
        $pdf->SetXY(10, 68);
        $pdf->MultiCell(70, 7,utf8_decode('Nro. de telefono fijo y celular:') , 1, 'L', FALSE);
        $pdf->SetXY(80, 68);
        $pdf->MultiCell(120, 7,utf8_decode("$detalleFormulario[4] - $detalleFormulario[5]") , 1, 'L', FALSE);
        $pdf->SetXY(10, 75);
        $pdf->MultiCell(70, 7,utf8_decode('Correo electrónico de contacto:') , 1, 'L', FALSE);
        $pdf->SetXY(80, 75);
        $pdf->MultiCell(120, 7,utf8_decode("$detalleFormulario[6]") , 1, 'L', FALSE);
        $pdf->SetXY(10, 82);
        $pdf->MultiCell(70, 14,utf8_decode('Dirección fiscal:') , 1, 'L', FALSE);
        $pdf->SetXY(80, 82);
        $pdf->MultiCell(120, 14,utf8_decode("$detalleFormulario[7]") , 1, 'L', FALSE);
        $pdf->SetXY(10, 96);
        $pdf->MultiCell(70, 7,utf8_decode('Fecha de la solicitud:') , 1, 'L', FALSE);
        $pdf->SetXY(80, 96);

        $dia = date("d", strtotime($detalleFormulario[8]));
        $mes = date("F", strtotime($detalleFormulario[8]));
        if ($mes == "January")
            $mes = "Enero";
        if ($mes == "February")
            $mes = "Febrero";
        ...
        if ($mes == "December")
            $mes = "Diciembre";
        $anio = date("Y", strtotime($detalleFormulario[8]));
        ...
        for ($j = 0; $j < sizeof($array_datos_ensayos); $j = $j + 9) {
            $ult_posic = $ult_posic +7;
            $pdf->SetXY(10, $ult_posic);
            $pdf->MultiCell(30, 7,$array_datos_ensayos[$j+5] , 1, 'C', FALSE);
            $pdf->SetXY(40, $ult_posic);
            $pdf->MultiCell(100, 7,utf8_decode($array_datos_ensayos[$j+4]) , 1, 'C', FALSE);
            $pdf->SetXY(140, $ult_posic);
            $pdf->MultiCell(30, 7,utf8_decode($array_datos_ensayos[$j+7]) , 1, 'C', FALSE);
            $pdf->SetXY(170, $ult_posic);
            $pdf->MultiCell(30, 7,utf8_decode($array_datos_ensayos[$j+6]) , 1, 'C', FALSE);
        }
        ...
        $pdf->MultiCell(30, 7,utf8_decode($detalleFormulario[9]) , 1, 'C', TRUE);

        $cod_solicitud = $detalleFormulario[11];
        ...
        echo "<script language='javascript'>window.open('Archivos/EnsayoLaboratorio/$cod_solicitud/Documentos/$nombreArchivo.pdf','_blank'); </script>";
        exit;
    }

    function imprimir_nota_aceptacion($idSolicitud, $porcentaje, $tipo_proyecto, $detalleFormulario) {
        $tipo_cliente = $detalleFormulario[10];
        $nombre_factura = $detalleFormulario[1];
        $nit_ci = $detalleFormulario[2];
        $nombre_contacto = $detalleFormulario[3];
        $telefono_fijo = $detalleFormulario[4];
        $telefono_celular = $detalleFormulario[5];
        $correo = $detalleFormulario[6];
        $direccion_fiscal = $detalleFormulario[7];
        $ci_contacto = $detalleFormulario[8];

        $detalleEnsayoDao = new DetalleEnsayoDAO();
        $array_datos_ensayos = $detalleEnsayoDao->getDetalleEnsayos($idSolicitud);
        $precio_total = $detalleFormulario[9];
        ...
        for ($j = 0; $j < sizeof($array_datos_ensayos); $j = $j + 9) {
            $ult_posic = $ult_posic +7;
            $pdf->SetY($ult_posic);
            $pdf->MultiCell(20, 7,utf8_decode($array_datos_ensayos[$j+5]) , 1, 'C', FALSE);
            $pdf->SetXY(45, $ult_posic);
            $pdf->MultiCell(90, 7,utf8_decode($array_datos_ensayos[$j+4]) , 1, 'C', FALSE);
            $pdf->SetXY(135, $ult_posic);
            $pdf->MultiCell(25, 7,utf8_decode($array_datos_ensayos[$j+7]) , 1, 'C', FALSE);
            $pdf->SetXY(160, $ult_posic);
            $pdf->MultiCell(25, 7,utf8_decode($array_datos_ensayos[$j+6]) , 1, 'C', FALSE);
        }
        ...
        $pdf->MultiCell(70, 6,utf8_decode('Facturación a nombre de (razón social de la empresa o nombre del particular):') , 1, 'L', FALSE);
        $pdf->SetXY(80, 42);
        $pdf->MultiCell(120, 12,utf8_decode("$nombre_factura") , 1, 'L', FALSE);
        $pdf->SetXY(10, 54);
        $pdf->MultiCell(70, 7,utf8_decode('Nro. de NIT o Nro. de CI:') , 1, 'L', FALSE);
        $pdf->SetXY(80, 54);
        $pdf->MultiCell(120, 7,utf8_decode("$nit_ci") , 1, 'L', FALSE);
        $pdf->SetXY(10, 61);
        $pdf->MultiCell(70, 7,utf8_decode('Nombre de la persona de contacto:') , 1, 'L', FALSE);
        $pdf->SetXY(80, 61);
        $pdf->MultiCell(120, 7,utf8_decode("$nombre_contacto") , 1, 'L', FALSE);
        $pdf->SetXY(10, 68);
        $pdf->MultiCell(70, 7,utf8_decode('Nro. de telefono fijo y celular:') , 1, 'L', FALSE);
        $pdf->SetXY(80, 68);
        $pdf->MultiCell(120, 7,utf8_decode("$telefono_fijo - $telefono_celular") , 1, 'L', FALSE);
        $pdf->SetXY(10, 75);
        $pdf->MultiCell(70, 7,utf8_decode('Correo electrónico de contacto:') , 1, 'L', FALSE);
        $pdf->SetXY(80, 75);
        $pdf->MultiCell(120, 7,utf8_decode("$correo") , 1, 'L', FALSE);
        $pdf->SetXY(10, 82);
        $pdf->MultiCell(70, 14,utf8_decode('Dirección fiscal:') , 1, 'L', FALSE);
        $pdf->SetXY(80, 82);
        $pdf->MultiCell(120, 14,utf8_decode("$direccion_fiscal") , 1, 'L', FALSE);
        $pdf->SetXY(10, 96);
        $pdf->MultiCell(70, 7,utf8_decode('Fecha de la solicitud:') , 1, 'L', FALSE);
        $pdf->SetXY(80, 96);
    ...
    function imprimir_nota_aceptacion_tc($id_ensayo, $porcentaje, $tipo_proyecto, $detalleFormulario) {
        $nombre_factura = $detalleFormulario[1];
        $nit_ci = $detalleFormulario[2];
        $nombre_contacto = $detalleFormulario[3];
        $telefono_fijo = $detalleFormulario[10];
        $telefono_celular = $detalleFormulario[11];
        $correo = $detalleFormulario[12];
        $direccion_fiscal = $detalleFormulario[13];
        $ci_contacto = $detalleFormulario[14];
        $codigo_proyecto = $detalleFormulario[15];

        $precio_total = $detalleFormulario[5];

        $ubicacion_especifica = $detalleFormulario[16];
        $nombre_proyecto = $detalleFormulario[0];
        ...
        $cod_solicitud = $detalleFormulario[15];
        $d = $destino = "../Archivos/EnsayoLaboratorio/$cod_solicitud/Documentos/";
        $nombreArchivo = "formulario-$cod_solicitud";
        if (file_exists($d)) {
